...
export default function ProfileView() {
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (authUser) {
      ...
      try {
        const response = await UpdatePasswordAction(authUser.id!, updatePassData);
        ...
        <Card title="Perfil de Usuario" header={header} style={{ width: '100%', maxWidth: '640px' }}>
          <div className="p-fluid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
            <div>
              ...
              <InputText id="email" value={authUser?.email || ''} readOnly style={{ fontWeight: 'bold' }} />
            </div>
            <div>
              <label htmlFor="cedula">Cédula</label>
              <InputText id="cedula" value={authUser?.identity || ''} readOnly style={{ fontWeight: 'bold' }} />
            </div>
            <div>
              <label htmlFor="nombre">Nombre</label>
              <InputText id="nombre" value={authUser?.name || ''} readOnly style={{ fontWeight: 'bold' }} />
            </div>
            <div>
              <label htmlFor="apellido">Apellido</label>
              <InputText id="apellido" value={authUser?.secondLastName || ''} readOnly style={{ fontWeight: 'bold' }} />
            </div>
          </div>
          <Divider />
          {errorMessage && (
            <Message severity="error" text={errorMessage} style={{ marginBottom: '16px' }} />
          )}
          {successMessage && (
            <Message severity="success" text={successMessage} style={{ marginBottom: '16px' }} />
          )}
