...
const estadosAsistencia: Asistencia[] = [
  "Presente",
  "Ausente",
  "Justificado",
  "No justificado",
...
export default function AsistenciaEstudiantes() {
  ...
  const [calendarioVisible, setCalendarioVisible] = useState<{
    [id: number]: boolean;
  }>({});
  ...
  const filtrarEstudiantes = () => {
    return estudiantes.filter((estudiante) => {
      const filtroAsistenciaAplica =
        filtroAsistencia === "Todos" ||
        Object.values(estudiante.asistencia).includes(filtroAsistencia);
      const filtroCursoAplica =
        filtroCurso === "Todos" || estudiante.curso === filtroCurso;
      const filtroMateriaAplica =
        filtroMateria === "Todos" || estudiante.materia === filtroMateria;
  ...
  const calcularTotales = (asistencia: { [fecha: string]: string }) => {
    ...
    const totalPresente = Object.values(asistencia).filter(
      (estado) => estado === "Presente"
    ).length;
    const totalAusente = Object.values(asistencia).filter(
      (estado) => estado === "Ausente"
    ).length;
    const totalJustificado = Object.values(asistencia).filter(
      (estado) => estado === "Justificado"
    ).length;
    const totalNoJustificado = Object.values(asistencia).filter(
      (estado) => estado === "No justificado"
    ).length;
    ...
    return {
      ...
      porcentajeJustificado: totalDias
        ? (totalJustificado / totalDias) * 100
        : 0,
      porcentajeNoJustificado: totalDias
        ? (totalNoJustificado / totalDias) * 100
        : 0,
    };
  };
            ...
            <>
              ...
              {calendarioVisible[rowData.id] && (
                <Calendar
                  ...
                  inline
                  dateTemplate={(event) =>
                    dateTemplate(event, rowData.asistencia)
                  }
                />
              )}
            </>
      ...
      {estudianteSeleccionado && (
        <div className="mt-4">
          <h2 className="text-xl font-bold">
            Resumen de Asistencia de {estudianteSeleccionado.nombre}
          </h2>
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  <div>
                    Total de Presente: {totales.totalPresente} (
                    {totales.porcentajePresente.toFixed(2)}%)
                  </div>
                  <div>
                    Total de Ausente: {totales.totalAusente} (
                    {totales.porcentajeAusente.toFixed(2)}%)
                  </div>
                  <div>
                    Total de Justificado: {totales.totalJustificado} (
                    {totales.porcentajeJustificado.toFixed(2)}%)
                  </div>
                  <div>
                    Total de No Justificado: {totales.totalNoJustificado} (
                    {totales.porcentajeNoJustificado.toFixed(2)}%)
                  </div>
                </>
              );
            })()}
