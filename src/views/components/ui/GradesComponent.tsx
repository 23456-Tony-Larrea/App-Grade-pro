...
const initialStudents = [
  { id: 1, name: 'Ana García', course: 'A', subject: 'Matemáticas', grades: [85, 90, 88, 92] },
  { id: 2, name: 'Carlos López', course: 'B', subject: 'Historia', grades: [78, 82, 0, 0] },
  { id: 3, name: 'Elena Martínez', course: 'C', subject: 'Ciencias', grades: [92, 95, 91, 89] },
  { id: 4, name: 'David Rodríguez', course: 'A', subject: 'Literatura', grades: [88, 85, 89, 0] },
  { id: 5, name: 'Sofía Hernández', course: 'B', subject: 'Matemáticas', grades: [95, 93, 97, 94] },
];
...
export default function GradeComponent({ title, icon }: GradeComponentProps) {
  ...
  const filteredStudents = students.filter(student => 
    (courseFilters.includes('Todos') || courseFilters.includes(student.course)) &&
    (subjectFilters.includes('Todos') || subjectFilters.includes(student.subject)) &&
    (nameFilter === '' || student.name.toLowerCase().includes(nameFilter.toLowerCase()))
  );

  const updateGrade = (studentId: number, gradeIndex: number, value: number) => {
    setStudents(students.map(student => 
      student.id === studentId 
        ? { 
            ...student, 
            grades: student.grades.map((grade, index) => 
              index === gradeIndex ? Math.max(0, Math.min(100, grade + value)) : grade
            ) 
          } 
        : student
    ));
  ...
  return (
    <div className="p-4">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1rem', marginBottom: '1rem' }} className="md:grid-cols-3">
        <div>
          <label htmlFor="courseSelect" style={{ fontSize: '1.125rem', fontWeight: '600' }}>Cursos</label>
          <Dropdown 
            value={courseFilters} 
        ...
        <div>
          <label htmlFor="subjectSelect" style={{ fontSize: '1.125rem', fontWeight: '600' }}>Materias</label>
          <Dropdown 
        ...
        <div>
          <label htmlFor="nameFilter" className="text-lg font-semibold">Buscar por nombre</label>
          <InputText
            id="nameFilter"
            type="text"
      ...
      <div className="mb-4 flex justify-end space-x-2" style={{ textAlign: 'right' }}>
        <Button
      ...
      <div className="overflow-x-auto">
        <DataTable value={filteredStudents}>
          ...
          {[...Array(gradeColumns)].map((_, index) => (
            <Column key={index} header={`Calificación ${index + 1}`} body={(rowData) => (
