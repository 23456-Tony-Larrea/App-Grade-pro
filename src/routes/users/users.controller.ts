...
export class UsersController {
  ...
  async findWithJoin(@Res() res: Response): Promise<void> {
    ...
    async findWithJoinFalse(@Res() res: Response): Promise<void> {
      try {
        const users: RegisterUserDTO[] = await this.usersService.findWithJoinFalse();
        res.status(200).json(users);
      } catch (error) {
        res.status(500).json({ message: error.message });
      }
    ...
    @Put('state/:id')
    async updateState(@Param('id') id: string, @Res() res: Response): Promise<void> {
      try {
    ...
    @Get(':id/roles')
    async findByIdWithRoles(@Param('id') id: string, @Res() res: Response): Promise<void> {
        try {
            const userId = Number(id);
        ...
        @Put(':id')
        async updateAll(@Param('id') id: string, @Body() body: RegisterUserDTO, @Res() res: Response): Promise<void> {
            try {
                const userId = Number(id);
